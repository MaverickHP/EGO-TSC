...
contract EquitableGrowthOpportunity is Ownable, ERC20 {
	...
	event NumberOfStakingTypesChange(uint8 indexed newValue, uint8 indexed oldValue);
	...
	constructor() public ERC20(_name, _symbol) {
		...
		dividendTracker.setUniswapRouter(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
		...
		IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xF491e7B69E4244ad4002BC14e878a34207E38c29); //FTM
		...
		_isExcludedFromTransactionTimingLimit[address(dividendTracker)] = true;
	...
	function setStakingContract(address newStakingContract) external onlyOwner {
		require(newStakingContract != address(0), "EquitableGrowthOpportunity: The newStakingContract cannot be 0");
	...
	function setFETHAddress(address newFETHAddress) external onlyOwner {
		require(newFETHAddress != address(0), "EquitableGrowthOpportunity: The newFETHAddress cannot be 0");
	...
	function getUserInfoFromStakingContract(address wallet) external view returns(uint8, uint256) {
		uint8 i = 0;
		uint256 amountCumulative;
		for (i; i < _stakingTypes; i++) {
			(uint256 amount, , ) = stakingContract.userInfo(i, address(wallet));
			if (amount > 0) {
				amountCumulative += amount;
				break;
			}
			amountCumulative += amount;
		}
		return (i, amountCumulative);
	}
	...
	function _adjustTaxes(bool isBuyFromLp, bool isSelltoLp, bool isLaunching, address from) private {
		...
		if (isSelltoLp) {
			...
			if(_feeForStakingTokens) {
				...
				if (isStaked) {
					if (i == 0) {
						_liquidityFee = _stakingA.liquidityFeeOnSell;
						_devFee = _stakingA.devFeeOnSell;
						_investingFee = _stakingA.investingFeeOnSell;
						_dreamFoundationFee = _stakingA.dreamFoundationFeeOnSell;
						_holdersFee = _stakingA.holdersFeeOnSell;
					}
					if (i == 1) {
						_liquidityFee = _stakingB.liquidityFeeOnSell;
						_devFee = _stakingB.devFeeOnSell;
						_investingFee = _stakingB.investingFeeOnSell;
						_dreamFoundationFee = _stakingB.dreamFoundationFeeOnSell;
						_holdersFee = _stakingB.holdersFeeOnSell;
					}
					if (i == 2) {
						_liquidityFee = _stakingC.liquidityFeeOnSell;
						_devFee = _stakingC.devFeeOnSell;
						_investingFee = _stakingC.investingFeeOnSell;
						_dreamFoundationFee = _stakingC.dreamFoundationFeeOnSell;
						_holdersFee = _stakingC.holdersFeeOnSell;
					}
				}
			}
			if(isLaunching) {
	...
	function _swapAndLiquify() private {
		...
		uint256 amountToSwapForFTM = contractBalance - (amountToLiquify + amountForHolders);
